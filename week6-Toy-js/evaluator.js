...
export class Evaluator {
  constructor() {
    ...
    // let result = this.evaluator(node);
  }
  ...
  WhileStatement(node) {
    while (true) {
      ...
      if (condition.toBoolean().value) {
        ...
        if (record.type == "break") return new CompletionRecord("normal");
      } else {
  ...
  NumericalLiteral(node) {
    let str = node.value;
    console.log("NumericalLiteral -> str", str);
    let l = str.length;
    let value = 0;
    let n = 10;
  ...
  PropertyList(node, object) {
    if (node.children.length === 1) {
      ...
      this.PropertyList(node.children[0], object);
      this.PropertyList(node.children[2], object);
    }
  }
  ...
  NumberExpression(node) {
    ...
    if (node.children.length === 3) {
      ...
      let prop = obj.et(node.children[2].name);
