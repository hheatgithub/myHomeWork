...
let syntax = {
  ...
  Arguments: [["(", ")"], ["ArgumentList", ",", "AssignmentExpression"]],
  ...
  while (queue.length) {
    ...
    if (syntax[symbol]) {
      for (let rule of syntax[symbol]) {
        ...
        //console.log("----symbol=", symbol, " current=", current);
      }
    }
  }
...
export function parse(source) {
  ...
  function reduce() {
    ...
    if (state.$reduceType) {
      ...
      return { type: state.$reduceType, children: children.reverse() };
    } else {
      console.log("state.$reduceType=", state.$reduceType);
      debugger;
